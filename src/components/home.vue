      ...
      return [
        {
          data: this.suggestions[0].data.filter(option => {
            return (
              option.name.toLowerCase().indexOf(this.query1.toLowerCase()) > -1
            );
          })
        }
      ];
      ...
      if (this.selected.id == this.selected1.id) {
       ...
       // console.log(this.msg);
        for (let i = 0; i < Object.keys(this.all_buses).length; i++) {
          firebase
            .database()
            .ref("/routes/bus_route/" + this.all_buses[0][i] + "/timings_stops")
            .once("value", snap => {
          ...
          firebase
            .database()
            .ref(
              "/routes/bus_route/" + this.all_buses[0][i] + "/timings_stops_1"
            )
            .once("value", snap => {
              //(Object.keys(snap.val()).length)
    ...
    getstop(data, bus, sel) {
      ...
      if (i < j) {
        if (sel == 1)
          firebase
            .database()
            .ref("/routes/bus_route/" + bus + "/timings_stops")
            .once("value", snap => {
        ...
        if (sel == 2) {
          ...
          firebase
            .database()
            .ref("/routes/bus_route/" + bus + "/timings_stops_1")
            .once("value", snap => {
